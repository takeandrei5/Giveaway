...
public sealed class ExecuteAsync_1 : Base
{
    ...
    public async Task Check_ExecuteAsync_Execution_Flow_If_Current_User_Could_Not_Be_Found()
    {
        ...
        _userServiceMock.Setup(userService => userService.FindUserByEmailAsync(Email, It.IsAny<CancellationToken>()))
        ...
        _messageServiceMock.Verify(messageService =>
                messageService.ReadConversationByUserEmailAsync(
                    ...
                    It.Is<UserEmail>(userEmail => userEmail.Value == Email), CancellationToken.None),
            Times.Never);
    }
}
